...
public class JoystickView extends View {
  ...
  protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
    ...
    int d = Math.min(getMeasuredWidth(), getMeasuredHeight())/3;
...
//                Log.d(TAG, "ACTION_POINTER_UP: " + pointerId);
                returnHandleToCenter();
                setPointerId(INVALID_POINTER_ID);
          //      SDLActivity.onNativeKeyUp(getPointerId());
              return true;
              }
          }
            break;
...
//        Log.d(TAG, "reset click");
        invalidate();
     //   SDLActivity.onNativeKeyUp(HapticFeedbackConstants.VIRTUAL_KEY);
      }
...
//        Log.d(TAG, "click");
    ...
    //    SDLSurface.onKey1(HapticFeedbackConstants.VIRTUAL_KEY); //(HapticFeedbackConstants.VIRTUAL_KEY);
